...
<#
.SYNOPSIS
Locates the path to the specified command.
...
function Which-Command (
    # Command to locate
    $command,
    # Throw (default) or return null if command cannot be located
    $throwOnNotFound = $true)
{
    [string]$path = get-command $command -Type Application -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Definition -First 1
    if ($path) { return $path }
    if ($throwOnNotFound) {throw "Command '$command' path not found"}
    return $null;
}
